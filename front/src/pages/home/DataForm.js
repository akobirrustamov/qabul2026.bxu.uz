...
function DataForm() {
  ...
  const handleSelectChange = async (selectedOption, { name }) => {
    ...
    if (name === "educationTypeId") {
      ...
      // API chaqiramiz
      const response = await ApiCall(
          `/api/v1/education-form/active/${value}`,
          "GET",
          null,
          null,
          true
      ...
      setEducationForm(forms);

      // 🔥 AGAR educationType == 2 bo‘lsa
      if (value === 2 && forms.length > 0) {
        const firstForm = forms[0];

        // avtomatik tanlash
        setAbuturient((prev) => ({
          ...prev,
          educationFormId: firstForm.value,
        }));

        // fieldni ham yuklaymiz
        getEducationField(firstForm.value);
      }
    }
      ...
      setAbuturient((prev) => ({
        ...prev,
        educationFormId: value,
        educationFieldId: "",
      }));

      getEducationField(value);
    }
                        ...
                        <div>
                          ...
                          <Select
                              ...
                              value={educationForm.find(
                                  (option) => option.value === abuturient.educationFormId
                              )}
                              onChange={handleSelectChange}
