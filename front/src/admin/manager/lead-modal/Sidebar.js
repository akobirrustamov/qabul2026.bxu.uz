...
function Sidebar({ onHoverChange }) {
  ...
  const isActive = (path) => {
    if (path === "/manager") return currentPath === "/manager/home";
  ...
  const staticRoutes = [
...

    {
      ...
      icon: <MdPeople className="h-5 w-5 flex-shrink-0" />,
    },
  ];
    ...
    >
                ...
                <div className="space-y-1">
                  {categories.filter((c) => c.status).length > 0
                    ? categories
                        ...
                        .map((category) => {
                          const active =
                            currentPath === `/manager/crms/${category.id}` ||
                            currentPath.startsWith(
                              `/manager/crms/${category.id}/`,
                            );
                          return (
      ...
      <div className="border-t border-white/5 p-3">
        <button
          ...
          title={isCollapsed ? "Ortga qaytish" : ""}
        ...
        >
          ...
          {!isCollapsed && (
            <span className="text-sm font-medium">Ortga qaytish</span>
          )}
        </button>
      </div>
    </div>
