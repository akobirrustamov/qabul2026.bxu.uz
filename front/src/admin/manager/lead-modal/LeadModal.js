...
export default function LeadModal({
  ...
  // Init reminder operator from lead operator
  useEffect(() => {
    if (!show || !lead) return;
    if (lead.operator) {
      setReminderOperator({
        value: lead.operator.id,
        label: lead.operator.name || lead.operator.id,
      });
    }
  }, [show, lead]);
  ...
  const handleSaveReminder = async () => {
    ...
    try {
      ...
      await ApiCall(`/api/v1/crm/leads/${lead.id}/${userId}`, "PUT", {
        ...
        operatorId: reminderOperator.value,
      });
  ...
  const handlePlayAudio = async (recordId) => {
    ...
    try {
      ...
      setAudioMap((prev) => ({ ...prev, [recordId]: url }));
      setPlayingId(recordId);
    } catch (e) {
  ...
  // Reminder select styles (higher z-index for panel)
      ...
      <div className="fixed inset-0 z-50 flex flex-col bg-white">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex flex-1 flex-col overflow-hidden p-3">
            ...
            {/* ── Reminder panel (above input area) ───────────────────────── */}
            {showReminderPanel && (
              <div
