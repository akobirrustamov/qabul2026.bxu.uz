...
import logo1 from "../../ambasador/logo.png"
import {
    User,
    Phone,
    Shield,
...
function AmbassadorProfile() {
    ...
    const fetchProfile = async () => {
        try {
            const res = await ApiCall(`/api/v1/agent/me2/${id}`, "GET", null, null, true);
            if (res.data) {
                setUser(res.data);
    ...
    const fetchCardDataStatistic = async (id) => {
        try {
            const res = await ApiCall(`/api/v1/ambassador/statistic/${id}`, "GET");
            if (res.data) {
    ...
    const fetchCardData = async (id) => {
        try {
            const res = await ApiCall(`/api/v1/ambassador-payment-data/${id}`, "GET");
        ...
        <div className="min-h-screen bg-gray-50">
            <div className="flex">
                ...
                <div className="flex-1 p-4 md:p-6 lg:p-8 ml-0 md:ml-64">
                    <div className="max-w-6xl mt-4 md:mt-1 mx-auto">
                                ...
                                <div className="mb-4">
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
                                        ...
                                        <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-3 md:p-4 rounded-lg md:rounded-xl border border-blue-200">
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    ...
                                                    <p className="text-lg md:text-xl font-bold text-blue-800">{formatNumber(statistics.totalCount)}</p>
                                                </div>
                                                <div className="p-1 md:p-2 bg-blue-100 rounded-md md:rounded-lg">
                                                    <DollarSign className="text-blue-600" size={16} />
                                                </div>
                                            </div>
                                            <p className="text-[10px] md:text-xs text-blue-500 mt-1 md:mt-2">
                                        ...
                                        <div className="bg-gradient-to-br from-green-50 to-green-100 p-3 md:p-4 rounded-lg md:rounded-xl border border-green-200">
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    <p className="text-xs md:text-sm text-green-600 font-medium">Tasdiqlangan</p>
                                                    <p className="text-lg md:text-xl font-bold text-green-800">{formatNumber(statistics.status3Count)}</p>
                                                </div>
                                                <div className="p-1 md:p-2 bg-green-100 rounded-md md:rounded-lg">
                                                    <CheckCircle className="text-green-600" size={16} />
                                                </div>
                                            </div>
                                            <p className="text-[10px] md:text-xs text-green-500 mt-1 md:mt-2">
                                                Summa: {formatNumber(statistics.status3Amount)} so'm
                                            </p>
                                        </div>

                                        {/* Kutilayotgan to'lovlar */}
                                        <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 p-3 md:p-4 rounded-lg md:rounded-xl border border-yellow-200">
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    <p className="text-xs md:text-sm text-yellow-600 font-medium">Kutilayotgan</p>
                                                    <p className="text-lg md:text-xl font-bold text-yellow-800">{formatNumber(statistics.status2Count)}</p>
                                                </div>
                                                <div className="p-1 md:p-2 bg-yellow-100 rounded-md md:rounded-lg">
                                                    <Clock className="text-yellow-600" size={16} />
                                                </div>
                                            </div>
                                            <p className="text-[10px] md:text-xs text-yellow-500 mt-1 md:mt-2">
                                                Summa: {formatNumber(statistics.status2Amount)} so'm
                                            </p>
                                        </div>

                                        {/* Rad etilgan to'lovlar */}
                                        <div className="bg-gradient-to-br from-red-50 to-red-100 p-3 md:p-4 rounded-lg md:rounded-xl border border-red-200">
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    <p className="text-xs md:text-sm text-red-600 font-medium">Rad etilgan</p>
                                                    <p className="text-lg md:text-xl font-bold text-red-800">{formatNumber(statistics.status1Count)}</p>
                                                </div>
                                                <div className="p-1 md:p-2 bg-red-100 rounded-md md:rounded-lg">
                                                    <AlertCircle className="text-red-600" size={16} />
                                                </div>
                                            </div>
                                            <p className="text-[10px] md:text-xs text-red-500 mt-1 md:mt-2">
