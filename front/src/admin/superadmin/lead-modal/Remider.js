    ...
    >
      <div
        ...
        onMouseLeave={(e) => {
          ...
          e.currentTarget.style.boxShadow = "0 2px 8px rgba(0,0,0,0.04), 0 1px 2px rgba(0,0,0,0.05)";
          ...
          <div className="flex items-start justify-between gap-3 mb-2.5">
            ...
            <div className="flex-shrink-0 text-right">
              <p
                ...
                style={{ color: s.text, fontFamily: "'JetBrains Mono', monospace" }}
              >
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            {!lead.completed && (
              <button
                ...
                className="flex-shrink-0 flex items-center gap-1 rounded-lg px-2.5 py-1 text-[11px] font-semibold text-white transition-all"
                style={{ background: "#22c55e", border: "1px solid #16a34a", color: "#ffffff" }}
                onMouseEnter={(e) => {
...
function Reminder() {
  ...
  const fetchReminders = useCallback(async () => {
    try {
      setLoading(true);
      const res = await ApiCall("/api/v1/crm/leads/reminders", "GET");
  ...
  const handleComplete = async (leadId) => {
    try {
      ...
      if (!userId) { toast.error("User topilmadi ❌"); return; }
  ...
  // Counts
  const counts = {
    overdue: reminders.filter((r) => urgencyLevel(r.reminderTime) === "overdue").length,
    urgent: reminders.filter((r) => urgencyLevel(r.reminderTime) === "urgent").length,
    today: reminders.filter((r) => urgencyLevel(r.reminderTime) === "today").length,
    later: reminders.filter((r) => urgencyLevel(r.reminderTime) === "later").length,
  };
  ...
  const filtered = reminders.filter((r) => {
    ...
    const matchSearch =
      name.includes(q) ||
      (r.phone || "").includes(q) ||
      (r.operator?.username || r.operator?.name || "").toLowerCase().includes(q) ||
  ...
  const FILTER_TABS = [
    ...
    { key: "overdue", label: "O'tgan", count: counts.overdue, color: "#dc2626" },
    { key: "urgent", label: "Shoshilinch", count: counts.urgent, color: "#f97316" },
    { key: "today", label: "Bugun", count: counts.today, color: "#eab308" },
    { key: "later", label: "Keyingi", count: counts.later, color: "#22c55e" },
  ];

  return (
    <div className="flex min-h-screen overflow-hidden" style={{ background: "#f8fafc" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&family=JetBrains+Mono:wght@400;500;600&display=swap');

        @keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
        .reminder-spin { animation: spin 0.9s linear infinite; }
      ...
      {/* Main */}
      <div className="ml-[72px] flex flex-1 flex-col min-h-screen" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
        ...
        >
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3.5">
              ...
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="#b45309" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
              </div>
              <div>
                <h1 className="text-lg font-bold text-gray-800 leading-tight">Eslatmalar</h1>
            ...
            <div className="flex items-center gap-2.5">
              <div className="hidden md:flex items-center gap-2">
                <StatPill count={counts.overdue} label="o'tgan" color="#991b1b" dot="#ef4444" />
                <StatPill count={counts.urgent} label="shoshilinch" color="#9a3412" dot="#fb923c" />
                <StatPill count={counts.today} label="bugun" color="#854d0e" dot="#facc15" />
                <StatPill count={counts.later} label="keyingi" color="#166534" dot="#4ade80" />
              </div>
              ...
              >
                <svg
                  ...
                  className={`h-3.5 w-3.5 text-gray-600 ${loading ? "reminder-spin" : ""}`}
          ...
          <div className="mt-4 flex items-center gap-3 flex-wrap">      
            ...
            <div className="relative flex-1 min-w-[200px] max-w-sm">
              ...
              <input
                ...
                className="w-full rounded-xl py-2 pl-9 pr-4 text-sm text-gray-700 placeholder-gray-400 focus:outline-none transition-all"
                style={{ background: "#ffffff", border: "1px solid #e2e8f0" }}
                onFocus={(e) => { e.currentTarget.style.borderColor = "#cbd5e1"; e.currentTarget.style.boxShadow = "0 0 0 3px rgba(203,213,225,0.2)"; }}
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-8 pt-5">
          {loading ? (
            ...
            <div className="flex flex-col gap-3">
              {[...Array(5)].map((_, i) => (
                <div key={i} className="h-28 rounded-2xl reminder-shimmer" style={{ opacity: 1 - i * 0.12 }} />
              ))}
            </div>
          ) : filtered.length === 0 ? (
            /* Empty state */
            <div className="flex flex-col items-center justify-center gap-4 py-24">
              ...
              <div className="text-center">
                <p className="text-sm font-semibold text-gray-600">
                  {search || filterLevel !== "all" ? "Natija topilmadi" : "Hozircha eslatmalar yo'q"}
                </p>
                <p className="mt-1 text-xs text-gray-500">
                  {search ? "Boshqa kalit so'z bilan qidiring" : "Eslatmalar qo'shilganda bu yerda ko'rinadi"}
                </p>
              </div>
              {(search || filterLevel !== "all") && (
                <button
                  onClick={() => { setSearch(""); setFilterLevel("all"); }}
                  className="rounded-xl px-4 py-2 text-xs font-semibold text-gray-600 transition-colors"
                  style={{ background: "#ffffff", border: "1px solid #e2e8f0" }}
                  onMouseEnter={(e) => { e.currentTarget.style.borderColor = "#cbd5e1"; e.currentTarget.style.background = "#f8fafc"; e.currentTarget.style.color = "#1e293b"; }}
                  onMouseLeave={(e) => { e.currentTarget.style.borderColor = "#e2e8f0"; e.currentTarget.style.background = "#ffffff"; e.currentTarget.style.color = "#475569"; }}
                >
                  Filtrlarni tozalash
                </button>
              )}
            ...
            <div className="grid gap-2.5" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(380px, 1fr))" }}>
              {filtered.map((lead, i) => (
                <div key={lead.id} className="reminder-card-enter" style={{ animationDelay: `${Math.min(i, 8) * 40}ms` }}>
                  <ReminderCard
                    lead={lead}
        ...
        {!loading && filtered.length > 0 && (
          <div className="border-t px-6 py-2.5 text-xs text-gray-500" style={{ borderColor: "#e2e8f0", background: "#ffffff" }}>
