...
const Column = React.memo(function Column({
  ...
  const selectedInThisColumn = leads.filter((l) =>
    selectedLeadIds.has(l.id),
  ).length;

  const allSelected =
    selectMode && leads.length > 0 && selectedInThisColumn === leads.length;

  const someSelected =
    selectMode &&
    selectedInThisColumn > 0 &&
    selectedInThisColumn < leads.length;
  return (
    <div
      ref={setNodeRef}
          ...
          <div className="flex items-center justify-center gap-1.5">
            {selectMode && (
              <input
                ...
                onChange={() => onSelectAllInColumn(col.id)}
                className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 cursor-pointer"
                title="Hammasini tanlash"
              />
            )}
            <h2 className="text-xs font-bold uppercase tracking-wider text-black truncate">
              {col.name}
            </h2>
            {/* ✅ FILTER ICON */}
            {!selectMode && (
              <button
                ...
                className="ml-1 flex items-center justify-center rounded-full bg-gray-200 p-1 text-gray-600 hover:bg-blue-100 hover:text-blue-600 transition"
                title="Filter"
              >
                <svg
          ...
          <p className="mt-0.5 text-xs text-gray-500">
            ...
            {selectMode && (
              <span className="ml-1 text-blue-600 font-medium">
                ({selectedLeadIds.size} tanlangan)
              </span>
            )}
          </p>
        </div>
...
function Index() {
  ...
  const [loadingSelectAllCols, setLoadingSelectAllCols] = useState(new Set());
  ...
  const handleClearAllFilters = useCallback(async () => {
    try {
      ...
      const sorted = [...columnsRef.current];
      ...
      console.error(err);
      toast.error("Filterlarni tozalashda xatolik");
    }
  }, [categoryId]);

  const handleApplyFilter = async (filter) => {
    const colId = filterModal.colId;
    ...
    try {
      const res = await ApiCall(
        `/api/v1/crm/leads/sub-category/${colId}/filter`,
        "POST",
        {
          ...filter,
          page: 0,
      ...
      if (!res || !res.data) {
        toast.error("Serverdan ma'lumot kelmadi");
        return;
      }
      ...
      setSelectedLeadIds((prev) => {
        ...
        // faqat kelgan 100 ta ni tanlaymiz
        (res.data.content || []).forEach((l) => next.add(l.id));

        return next;
      });
    } catch (err) {
      console.error(err);
      toast.error("Filterda xatolik");
    }
  };
  // ── Refs ──────────────────────────────────────────────────────────────────
  ...
  const handleSelectAllInColumn = useCallback(
    async (colId) => {
      const data = columnData[colId];
      const leads = data?.leads || [];

      const hasFilter = !!filters[colId]; // 🔥 filter bor yoki yo‘q

      // 🔥 AGAR FILTER BOR BO‘LSA
      if (hasFilter) {
        setSelectedLeadIds((prev) => {
          const next = new Set(prev);
        ...
        return; // ❗ API ga bormaydi
      }

      // 🔥 AGAR FILTER YO‘Q BO‘LSA (eski holat)
      try {
        const res = await ApiCall(
          `/api/v1/crm/leads/sub-category/${colId}/ids`,
          "GET",
        );
        ...
        console.error(err);
        toast.error("Barcha leadlarni tanlashda xatolik");
      }
    },
    [columnData, filters],
  );
    ...
    <div className="flex h-screen select-none overflow-hidden font-sans">
          ...
          <div className="flex items-center gap-4">
            ...
            {Object.keys(filters).length > 0 && (
              <button
                ...
                className="px-3 py-1 rounded-xl bg-gray-700 text-white text-sm hover:bg-gray-800 transition-colors"
              >
                Filterni tozalash
              </button>
            )}
      ...
      {filterModal.open && (
        <FilterModal
          ...
          currentFilter={filters[filterModal.colId]}
          colName={filterModal.colName}
        />
      )}
