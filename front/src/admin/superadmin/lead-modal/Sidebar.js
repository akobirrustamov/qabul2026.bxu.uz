...
function Sidebar({ onHoverChange }) {
  ...
  const staticRoutes = [
    ...
    {
      name: "Operator Statistika",
      path: "/main/operator",
      icon: <MdBarChart className="h-5 w-5 flex-shrink-0" />,
    },
    {
      name: "Eslatmalar",
      path: "/main/crm-remider",
                ...
                <div className="space-y-1">
                  {categories.filter((c) => c.status).length > 0
                    ? categories
                        ...
                        .map((category) => {
                          const active =
                            currentPath === `/main/crms/${category.id}` ||
                            currentPath.startsWith(
                              `/main/crms/${category.id}/`,
                            );
                          return (
