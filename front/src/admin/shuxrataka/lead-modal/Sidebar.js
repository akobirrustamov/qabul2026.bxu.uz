...
import { MdHome, MdCategory, MdLogout, MdMenu, MdInbox } from "react-icons/md";
import {
  DndContext,
  closestCenter,
  KeyboardSensor,
                ...
                <div className="space-y-1">
                  {categories.filter((c) => c.status).length > 0
                    ? categories
                        ...
                        .map((category) => {
                          const active =
                            currentPath === `/dashboard/crms/${category.id}` ||
                            currentPath.startsWith(
                              `/dashboard/crms/${category.id}/`,
                            );
                          return (
