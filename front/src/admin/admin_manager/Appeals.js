...
function Appeals() {
  ...
  const handleEditClick = (appeal) => {
    ...
    setEditData({
      ...
      educationTypeId: appeal.educationType?.id || "",
      educationFormId: appeal.educationForm?.id || "",
  ...
  const validateInputs = () => {
    // Asosiy majburiy maydonlar (fatherName endi kerak emas)
    ...
    // Extra maydonlarga kiritish amalga oshirilganmi?
    const passportPinTouched = !!editData.passportPin?.trim();
    const passportNumberTouched = !!editData.passportNumber?.trim();
    const descriptionTouched = !!description.trim();
    const documentStatusTouched =
      documentStatus !== null && documentStatus?.value !== 0;

    const extraFieldsTouched =
      passportPinTouched ||
      passportNumberTouched ||
      descriptionTouched ||
      documentStatusTouched;

    // Extra maydonlarga to'liq kiritilganmi (agar foydalanuvchi boshlagan bo'lsa)
