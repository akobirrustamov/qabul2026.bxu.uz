...
//            return sendSmsCode(save, abuturient);
    ...
    public Integer contractNumber() {
        Integer randomPathNumber = ThreadLocalRandom.current().nextInt(1000000, 10000000);
    ...
    public void getAllAbuturientContract(@PathVariable String phone, HttpServletResponse response) throws IOException {
        ...
        Document document = new Document(PageSize.A4, 30, 30, 2, 28);
...
//            // 🎨 1. WATERMARK (САМОЕ СИЛЬНОЕ 🔥)
//            PdfContentByte canvas = writer.getDirectContentUnder();
//
//            PdfGState gs = new PdfGState();
//            gs.setFillOpacity(0.1f); // 🔥 прозрачность (0.0 - 1.0)
//
//            canvas.saveState();
//            canvas.setGState(gs);
//
//            // Загружаем шрифт для watermark текста
//            BaseFont bf = BaseFont.createFont(BaseFont.TIMES_ROMAN, BaseFont.WINANSI, BaseFont.NOT_EMBEDDED);
//
//            canvas.setFontAndSize(bf, 240); // размер шрифта — подбери под нужный
////            canvas.setFontAndSize(bf, 350); // размер шрифта — подбери под нужный
//            canvas.setColorFill(BaseColor.GRAY);
//
//// Поворот на 30° и позиционирование по центру страницы
//            canvas.beginText();
//            canvas.showTextAligned(
//                    Element.ALIGN_CENTER,
//                    "BXU",
//                    297,  // X: центр A4 (595/2)
////                    425,  // X: центр A4 (595/2)
//                    330,  // Y: примерно середина страницы
////                    420,  // Y: примерно середина страницы
//                    0   // угол поворота в градусах (0 = горизонтально)
//            );
//            canvas.endText();
//
//            canvas.restoreState();
            ...
            // 🧾 2. УЛУЧШЕННЫЙ HEADER (с линией)
            LineSeparator line = new LineSeparator();
            line.setLineWidth(1f);
            line.setLineColor(BaseColor.BLACK);
            document.add(new Chunk(line));
// Добавь пустой параграф с минимальным отступом
            Paragraph spacer = new Paragraph(" ");
            spacer.setSpacingBefore(0f);
            spacer.setSpacingAfter(0f);
            spacer.setLeading(1f);  // ← ключевое, минимальная высота строки
            document.add(spacer);

            PdfPTable headerTable = new PdfPTable(4);
            headerTable.setWidthPercentage(100);
            headerTable.setWidths(new int[]{1, 2, 3, 2});

            // ====== 1. LOGO CELL ======
            PdfPCell logoCell = new PdfPCell();
            // 📐 4. УЛУЧШЕНИЕ ТАБЛИЦЫ HEADER - убираем границы
            ...
            paymentCell.addElement(new Paragraph("(to’lov qilish vaqtida namuna)", italicFont));
            paymentCell.addElement(new Paragraph("TO'LOV MAQSADI:", boldFont));
            ...
            qrCell.setHorizontalAlignment(Element.ALIGN_RIGHT); // RIGHT yoki CENTER
            qrCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
//            qrCell.setPaddingTop(0);
//            qrCell.setPaddingBottom(0);
            qrCell.setPadding(0);
            qrCell.setUseAscender(true);
            qrCell.setUseDescender(true);
            ...
            try {
                ...
                int qrSize = 100; // KATTALASHTIRILDI
                ...
                qrImage.scaleToFit(65, 65); // NORMAL SIZE
                qrImage.setAlignment(Image.ALIGN_RIGHT); // yoki CENTER
...
// Добавь пустой параграф с минимальным отступом
            ...
            spacer2.setSpacingBefore(0f);
            spacer2.setSpacingAfter(0f);
            spacer2.setLeading(1f);  // ← ключевое, минимальная высота строки
            document.add(spacer2);

            Paragraph paragraph2 = new Paragraph();
            paragraph2.setSpacingBefore(2f);
            paragraph2.setAlignment(Element.ALIGN_JUSTIFIED);
            paragraph2.add(new Chunk("O'zbekiston Respublikasi Prezidentining 2021 yil 22-iyundagi PQ-5157 son qarori, Vazirlar Mahkamasining 2025 yil 13 sentabr 578-sonli Qarorida va universitet kengashining tegishli qarori asosida, bir tomondan ", regularFont));
            paragraph2.add(new Chunk("BUXORO XALQARO UNIVERSITETI", boldFont));
            paragraph2.add(new Chunk(" (keyingi o' rinlarda \"", regularFont));
            paragraph2.add(new Chunk("Ta'lim muassasasi", boldFont));
            paragraph2.add(new Chunk("\") nomidan rektor ", regularFont));
            paragraph2.add(new Chunk("Baratov Sharif Ramazonovich", boldFont));
            paragraph2.add(new Chunk(", ikkinchi tomondan talabalikka tavsiya etilgan talabgor ", regularFont));
            paragraph2.add(new Chunk(fullName, boldFont));
            paragraph2.add(new Chunk(" (keyingi o'rinlarda \"", regularFont));
            paragraph2.add(new Chunk("Ta'lim oluvchi", boldFont));
            paragraph2.add(new Chunk("\"), birgalikda \"tomonlar\" deb ataladigan shaxslar mazkur shartnomani quyidagicha tuzdilar.", regularFont));
            paragraph2.setLeading(11f, 0);
            document.add(paragraph2);

            Paragraph paragraph3 = new Paragraph("I. SHARTNOMA PREDMETI", boldFont);
            paragraph3.setSpacingBefore(1f);
            ...
            paragraph5.setSpacingBefore(1f);
            paragraph5.setAlignment(Element.ALIGN_CENTER);
            document.add(paragraph5);

            Paragraph paragraph7 = new Paragraph();
            paragraph7.setSpacingAfter(1f);
            paragraph7.setAlignment(Element.ALIGN_JUSTIFIED);

            paragraph7.add(new Chunk("        2.1 \"Ta'lim muassasasi\"da o'qish davrida ta'lim xizmatini ko'rsatish narxi universitet haqiqiy xarajatlarining kalkulyatsiyasi asosida hisoblanadi.\n", regularFont));
            paragraph7.add(new Chunk("        2.2 Ushbu shartnoma bo'yicha ta'lim oluvchini bir o'quv yili davomida o'qitish uchun to'lov miqdori ", regularFont));
            paragraph7.add(new Chunk(abuturient.getEducationField().getPrice().toString() + " so'mni", boldFont));
            paragraph7.add(new Chunk(" (stipendiyasiz) tashkil etadi va ushbu to'lov miqdorining 50 % har o'quv yilining 1-oktyabr kuniga qadar, qolgan qismi esa keyingi yilning 1-mayigacha to'lanishi shart. Bunda to'lov miqdorini hisoblash ta'lim oluvchini talabalikka qabul qilingan kundan boshlab hisoblanadi.\n", regularFont));
            paragraph7.add(new Chunk("        2.3 Talabalar orasidan joriy o'quv yili (sentyabrdan-iyul oyigacha) davrida o'qishini boshqa davlat va nodavlat oliy ta'lim muassasalariga ko'chirishi yoki o'z xoxishiga binoan talabalik safidan chetlashtirish maqsadida murojaat qilganda, shartnomada belgilangan bir yillik to'lov-kontrakt miqdori O'zbekiston Respublikasi Oliy va o'rta maxsus ta'lim vazirining 2012 yil 28 dekabr kunidagi 508- sonli buyrug'ida ko'rsatilgan tartibda talabadan saqlanib qolinadi.", regularFont));
            paragraph7.setLeading(11f, 0);
            ...
            Paragraph paragraph9_b = new Paragraph("   3.1. Ta'lim muassasasi majburiyatlari:\n", boldFont );
            document.add(paragraph9_b);

            Paragraph paragraph9 = new Paragraph();
            paragraph9.setSpacingAfter(1f);
            paragraph9.setAlignment(Element.ALIGN_JUSTIFIED);
            paragraph9.add(new Chunk(
                    "         -O'qitish uchun belgilangan dastlabki to'lov 25% dan kam bo'lmagan miqdorida amalga oshirgandan so'ng, \"Ta'lim oluvchi\"ni buyruq asosida talabalikka qabul qilish;\n" +
                            ...
                            "         -Respublikada belgilangan Mehnatga haq to'lashning eng kam miqdori yoki sifatli ta'lim xizmatlari ko'rsatish bilan bog'liq tariflar o'zgargan taqdirda o'qitish uchun belgilangan to'lov miqdori universitet kengashi qarori asosida ta'lim oluvchini 1 oy oldin universitet ijtimoiy tarmoqlarida e’lon berish orqali xabardor qilish.", regularFont));
            paragraph9.setLeading(11f, 0);
            document.add(paragraph9);

            Paragraph paragraph10_b = new Paragraph("   3.2. Ta'lim oluvchining majburiyatlari:\n" , boldFont );
            document.add(paragraph10_b);

            Paragraph paragraph10 = new Paragraph();
            paragraph10.setAlignment(Element.ALIGN_JUSTIFIED);
            paragraph10.add(new Chunk("         -Shartnomaning 2.2. bandida belgilangan to'lov summasini shu bandda ko'rsatilgan muddatlarda to'lab borish;\n" +
                    ...
                    "         -O'qitish uchun belgilangan to'lov miqdorini to'laganlik to'g'risidagi bank tasdiqnomasi, bakalavr diplomi va ilovasivaning asl nusxasi hamda shartnomaning bir nusxasini rasmiylashtirish uchun ta'lim muassasasiga topshirish;\n" +
            ...
            paragraph10.setSpacingAfter(1f);
            paragraph10.setLeading(11f, 0);
            document.add(paragraph10);

            Paragraph paragraph11 = new Paragraph("IV. TOMONLARNING HUQUQLARI", boldFont);
            paragraph11.setSpacingBefore(2f);
            paragraph11.setAlignment(Element.ALIGN_CENTER);
            document.add(paragraph11);

            Paragraph paragraph12 = new Paragraph();
            paragraph12.setAlignment(Element.ALIGN_JUSTIFIED);
            paragraph12.add(new Chunk("    4.1. Talim muassasasi huquqlari:\n" , boldFont));
            paragraph12.add(new Chunk( "         -O'quv jarayonini mustaqil ravishda amalga oshirish, \"Ta'lim oluvchi\"ning oraliq va yakuniy nazoratlarni topshirish, qayta topshirish tartibi hamda vaqtlarini belgilash;\n" +
            ...
            paragraph13.add(new Chunk("         -Talaba o'quv intizomiy va oliy ta'lim muassasasining ichki tartib-qoidalarini buzganligi, bir semester davomida darslarni uzrli sabablarsiz 74 (yetmish to'rt) soatdan ortiq qoldirganligi yoki o'qitish uchun belgilangan miqdordagi to'lovni o'z vaqtida amalga oshirmaganligi sababli talabalar safidan chetlashtirilganda yoxud belgilangan muddatlarda fanlarni o'zlashtira olmaganligi (akademik qarzdor bo'lganligi) sababli kursdan qoldirilganda oliy ta'lim muassasasi tomonidan uning tegishli o'quv semestri uchun amalga oshirilgan to'lovi qaytarib berilmaydi.\n" +
                    ...
                    , regularFont));
            paragraph13.add(new Chunk( "    4.2. Ta'lim oluvchining huquqlari:\n", boldFont ));
            paragraph13.add(new Chunk( "         -O'quv yili uchun shartnoma summasini semestrlarga yoki choraklarga bo'lmasdan bir yo'la to'liqligicha to'lash;\n" +
            ...
            paragraph13.setSpacingAfter(1f);
            paragraph13.setLeading(11f, 0);
            ...
            paragraph_5.setSpacingBefore(2f);
            paragraph_5.setAlignment(Element.ALIGN_CENTER);
            document.add(paragraph_5);

            Paragraph paragraph_5_b = new Paragraph(
                    "        5.1. Ushbu shartnoma ikki tomonlama imzolangandan so'ng yoki ta`lim oluvchi tomonidan to’lov amalga oshirilgan vaqtdan boshlab kuchga kiradi hamda o'quv davri tugagunga qadar amalda bo'ladi.\n" +
            ...
            paragraph_5_b.setSpacingAfter(1f);
            paragraph_5_b.setAlignment(Element.ALIGN_JUSTIFIED);
            ...
            paragraph_6.setSpacingBefore(2f);
            paragraph_6.setAlignment(Element.ALIGN_CENTER);
            document.add(paragraph_6);

            Paragraph paragraph_6_b = new Paragraph();
            paragraph_6_b.setSpacingAfter(1f);
            paragraph_6_b.setAlignment(Element.ALIGN_JUSTIFIED);
            paragraph_6_b.add(new Chunk("        6.1. Ushbu shartnomani bajarish jarayonida kelib chiqishi mumkin bo'lgan nizo va ziddiyatlar tomonlar o'rtasida muzokaralar olib borish yo'li bilan hal etiladi.\n" +
                    ...
                    "        6.6. Qabul komissiyasiga taqdim etilgan ", regularFont));
            paragraph_6_b.add(new Chunk("hujjatlarning haqqoniyligi", boldFont));
            paragraph_6_b.add(new Chunk(" o'rnatilgan tartibda tasdiqlangandan so'ng talabgor tegishli buyruq asosida talabalar safiga qabul qilinadi.", regularFont));
            paragraph_6_b.setLeading(11f, 0);
            document.add(paragraph_6_b);

            Paragraph paragraph_7 = new Paragraph("VII. TOMONLARNING REKVIZITLARI VA IMZOLARI", boldFont);
            paragraph_7.setSpacingBefore(1f);
            ...
            endTable.setWidths(new float[]{1.85f, 1.15f}); // увеличиваем правую ячейку за счёт левой
            ...
            leftCell1.addElement(new Paragraph("Telefon raqami: 55-309-99-99, 55-305-55-55, 99-773-17-37, 94-322-57-75, tg: shuxrat221_004", regularFont));
            ...
            Font largerBoldFont = new Font(Font.FontFamily.TIMES_ROMAN, 10, Font.BOLD);
            Font largerRegularFont = new Font(Font.FontFamily.TIMES_ROMAN, 10, Font.NORMAL);

            Paragraph hisob = new Paragraph();
            hisob.add(new Chunk("Hisob raqami: ", largerRegularFont));
            hisob.add(new Chunk("20208000105439719001", largerBoldFont));
            leftCell1.addElement(hisob);

            PdfPTable rectorTable = new PdfPTable(2);
            rectorTable.setWidths(new float[]{1, 2});

            PdfPCell rectorTextCell = new PdfPCell(new Paragraph("Rektor: Sh.R.Barotov", boldFont));
            ...
            logoCell1.setPaddingLeft(-25f);
            logoCell1.setPaddingTop(-50f);

            try {
                Image logo1 = Image.getInstance("./logo1.png");
                logo1.scaleToFit(170, 170);
            ...
            rightCell1.addElement(new Paragraph("F.I.O: _________________________________________", regularFont));
            rightCell1.addElement(new Paragraph("Ta'lim oluvchining imzosi: (_______________________)", regularFont));
            endTable.addCell(rightCell1);

            document.add(endTable);
            // 🎨 3. ОРНАМЕНТ (низ)
            addOrnamentRevert(document);
    ...
    @GetMapping("/contract/{phone}/{userId}")
    public void getAllAbuturientContractUser(@PathVariable String phone, HttpServletResponse response, @PathVariable UUID userId) throws IOException {
        Abuturient abuturient = abuturientRepo.findByPhone(phone);
        abuturient.setStatus(4);
        abuturientRepo.save(abuturient);
        if (abuturient == null) {
        ...
        CrmLeadComment comment = CrmLeadComment.builder()
                .crmLead(savedLead)
                .commenter(commenter)
                .description(commenter.getName() + " tomonidan shartnoma berildi ")
                .historyStatus(8)
                .createdAt(LocalDateTime.now())
                .build();
        CrmLeadComment saved = crmLeadCommentRepo.save(comment);
...
// Добавь пустой параграф с минимальным отступом
    ...
    // 🎨 3. ОРНАМЕНТ функцияси
    public void addOrnament(Document document) {
        try {
            Image ornament = Image.getInstance("./ornament.png");
            ornament.scaleToFit(400, 40);
            ornament.setAlignment(Image.ALIGN_CENTER);
            document.add(ornament);
        } catch (Exception e) {
            // Орнамент файли топилмаса, хатоликни игнор қиламиз
            e.printStackTrace();
        }
    }
    public void addOrnamentRevert(Document document) {
        try {
            Image ornament = Image.getInstance("./ornamentRevert.png");
            ornament.scaleToFit(400, 40);
            ornament.setAlignment(Image.ALIGN_CENTER);
            document.add(ornament);
        } catch (Exception e) {
